      ...
      try {
        ...
        if (selectError && selectError.code !== "PGRST116") {
         ...
         if (!existingProfile) {
           ...
           // Create admin profile using converter
           const adminProfile = {
             id: data.user.id,
             email: ADMIN_EMAIL,
             displayName: "Admin",
           ...
           const dbAdminProfile = typescriptUserToDatabaseUser(adminProfile);
           
           const { data: insertedProfile, error: insertError } = await supabase
             .from("users")
             .insert([dbAdminProfile])
             .select()
             .single();
           ...
           // Ensure existing profile has admin role using converter
           const updates = {
             role: "admin",
             subscriptionStatus: "active",
             updatedAt: new Date().toISOString(),
       ...
       if (paymentInfo) {
         try {
           ...
           const paymentRequestObj = {
             ...
             plan: paymentInfo.plan as SubscriptionPlan,
      ...
      try {
        ...
        if (!createProfileResponse.ok) {
          ...
          console.error("[completeEmailSignUp] ❌ Profile creation API error:", errorData);
           ...
           // Try alternative method - direct insert using converter
           try {
             const profileObj: Partial<User> = {
               id: newUser.id,
               email,
               displayName,
               phone,
